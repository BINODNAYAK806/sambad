...
function migrateSchema() {
  ...
  for (const table of syncTables) {
    const tableCols = db.prepare(`PRAGMA table_info(${table})`).all().map((col: any) => col.name);
    
    if (!tableCols.includes('is_deleted')) {
      console.log(`[Sambad DB] Adding is_deleted to ${table}`);
      db.exec(`ALTER TABLE ${table} ADD COLUMN is_deleted INTEGER DEFAULT 0`);
    }
    
    if (!tableCols.includes('last_updated_at')) {
      console.log(`[Sambad DB] Adding last_updated_at to ${table}`);
      // SQLite doesn't allow CURRENT_TIMESTAMP in ALTER TABLE ADD COLUMN
      db.exec(`ALTER TABLE ${table} ADD COLUMN last_updated_at TEXT`);
      db.exec(`UPDATE ${table} SET last_updated_at = CURRENT_TIMESTAMP WHERE last_updated_at IS NULL`);
    }
    
    if (!tableCols.includes('last_synced_at')) {
      console.log(`[Sambad DB] Adding last_synced_at to ${table}`);
...
export const campaigns = {
  ...
  getRecentActivity: (limit: number = 10): any[] => {
    ...
    return db.prepare(`
      ...
      LIMIT ?
    `).all(limit, limit, limit, limit) as any[];
  },
};
