...
export class WhatsAppClientSingleton {
    ...
    async getNumberId(serverId: number, number: string): Promise<any> {
        ...
        try {
            ...
            if (!results || results.length === 0) {
                console.warn(`[WhatsApp] [Server ${serverId}] ${jid} - NOT on WhatsApp (onWhatsApp returned empty)`);
                return null;
            }
            ...
            console.error(`[WhatsApp] [Server ${serverId}] Error checking number ${number}:`, err);
            return null;
        }
    }
    ...
    private formatJid(phoneNumber: string): string {
        ...
        // Handle common Indian number formats if specifically 10 digits
        // Default to Indian country code (91) for 10-digit numbers
        if (cleaned.length === 10) {
            console.log(`[WhatsApp] Auto-detected 10-digit number ${cleaned}, prepending country code 91`);
            cleaned = '91' + cleaned;
        } else if (cleaned.length === 11 && cleaned.startsWith('0')) {
            // Handle 09898... formats
            console.log(`[WhatsApp] Auto-detected leading 0 for 11-digit number ${cleaned}, replacing with 91`);
