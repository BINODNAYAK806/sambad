...
class CampaignExecutor {
    ...
    async sendMessage(message, serverId, campaignTask, runId = null) {
        ...
        try {
            ...
            if (!numberId) {
                console.error(`[CampaignExecutor] ❌ Validation failed for ${number} on Server ${serverId}. Number not found on WhatsApp.`);
            ...
            if (campaignTask.isPoll && campaignTask.pollQuestion && campaignTask.pollOptions) {
                ...
                if (!contentSent) {
                    const textMessage = await this.whatsAppClient.sendMessage(serverId, chatId, content);
