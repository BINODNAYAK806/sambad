...
class AuthService {
    ...
    getAdminClient() {
        const url = supabase_1.currentSupabaseUrl ||
        ...
        const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY ||
            ...
            supabase_1.currentSupabaseKey;
        if (!url || !serviceKey) {
            const missing = [];
            if (!url)
                missing.push('Supabase URL (VITE_SUPABASE_URL or SAMBAD_ACCOUNT_ID)');
            if (!serviceKey)
                missing.push('Service Role Key (SUPABASE_SERVICE_ROLE_KEY)');
    ...
    async register(data) {
        ...
        try {
            ...
            this.startHeartbeat();
            return { success: true, user: authData.user || authData.user };
        }
        catch (error) {
    ...
    async login(email, password) {
        ...
        if (profileError)
            throw profileError;
        if (!profile)
            throw new Error('Account setup incomplete. Please register again with the same email.');
