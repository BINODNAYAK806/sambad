...
exports.reports = exports.logs = exports.campaignMedia = exports.campaignMessages = exports.campaigns = exports.groups = exports.contacts = exports.currentSupabaseKey = exports.currentSupabaseUrl = exports.supabase = void 0;
...
exports.currentSupabaseUrl = null;
exports.currentSupabaseKey = null;
// --- Initialization ---
async function customFetch(url, init) {
    const urlString = typeof url === 'string' ? url : url instanceof URL ? url.toString() : url.url;
    try {
        const response = await (0, node_fetch_1.default)(urlString, { ...init, agent: httpsAgent, signal: init?.signal });
        return response;
    }
    catch (error) {
...
function initializeSupabase(customUrl, customKey) {
    ...
    exports.currentSupabaseUrl = supabaseUrl;
    exports.currentSupabaseKey = supabaseKey;
    try {
...
async function getTenantContext() {
    ...
    if (!companyId)
        throw new Error('No company context. Please login.');
...
exports.contacts = {
    ...
    listWithGroups: async () => {
        ...
        return (data || []).map((contact) => ({
            ...
            groups: (contact.group_contacts || []).map((gc) => gc.groups).filter((g) => g !== null)
        }));
    },
    ...
    removeDuplicates: async () => {
        ...
        const seen = new Set();
        const idsToDelete = [];
        (data || []).forEach(c => { if (seen.has(c.phone_number))
...
exports.groups = {
    ...
    addContact: async (groupId, contactId) => {
        const { client, companyId } = await getTenantContext();
    ...
    bulkAddContacts: async (groupId, contactIds) => {
        const { client, companyId } = await getTenantContext();
    ...
    getContacts: async (groupId) => {
        ...
        if (error)
            throw error;
        return (data || []).map((r) => ({ ...r.contacts, variables: r.contacts?.vars_json || undefined })).filter(c => c.id);
    },
    ...
    bulkAddContactsToMultipleGroups: async (groupIds, contactIds) => {
        const { client, companyId } = await getTenantContext();
        const records = [];
...
exports.campaigns = {
    ...
    create: async (campaign) => {
        ...
        const dbCampaign = { ...campaign, company_id: companyId };
    ...
    clearContacts: async (campaignId) => {
        const { client, companyId } = await getTenantContext();
...
exports.campaignMessages = {
    ...
    getFailed: async (campaignId) => {
        ...
        if (error)
            throw error;
        return (data || []).map((m) => ({ campaign_id: m.campaign_id, recipient: m.recipient_number, name: m.recipient_name || m.contacts?.name || 'Unknown', error: m.error_message || 'Unknown', failed_at: m.updated_at }));
    },
    updateStatus: async (messageId, status, errorStr) => {
        const { client, companyId } = await getTenantContext();
        const updateData = { status, updated_at: new Date().toISOString() };
        if (status === 'sent')
            updateData.sent_at = new Date().toISOString();
        if (errorStr)
            updateData.error_message = errorStr;
