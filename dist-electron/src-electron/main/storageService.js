...
class StorageService {
    ...
    initialize(config) {
        ...
        try {
            ...
            if (this.localDb) {
                errorLogger_1.ErrorLogger.info('[Storage] Local SQLite database initialized');
            }
            else {
                errorLogger_1.ErrorLogger.warn('[Storage] Local database not available (better-sqlite3 not installed)');
            }
        }
        catch (error) {
            errorLogger_1.ErrorLogger.error('[Storage] Failed to initialize local database', error);
            if (this.mode === 'local') {
                throw error;
            }
        ...
        if (config.supabaseUrl && config.supabaseKey) {
            try {
                ...
                errorLogger_1.ErrorLogger.info('[Storage] Supabase client initialized (optional mode)', { mode: this.mode });
            }
            catch (error) {
                errorLogger_1.ErrorLogger.error('[Storage] Failed to initialize Supabase', error);
                this.supabase = null;
            }
        }
    }
    ...
    async getContacts() {
        try {
            if (this.isCloudAvailable()) {
                ...
                if (error)
                    throw error;
                errorLogger_1.ErrorLogger.debug('[Storage] Retrieved contacts from cloud', { count: data.length });
                return data || [];
            }
        }
        catch (error) {
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud fetch failed, using local fallback', error);
        }
        // Local fallback
        if (!this.localDb) {
            errorLogger_1.ErrorLogger.error('[Storage] No storage available (cloud failed and local DB not initialized)');
            return [];
        }
        const contacts = db.contacts.list();
        errorLogger_1.ErrorLogger.debug('[Storage] Retrieved contacts from local DB', { count: contacts.length });
        return contacts;
    }
    async createContact(contact) {
        ...
        try {
            if (this.isCloudAvailable()) {
                ...
                if (error)
                    throw error;
                errorLogger_1.ErrorLogger.info('[Storage] Contact created in cloud', { phone: contact.phone });
                return data;
            }
        }
        catch (error) {
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud create failed, using local fallback', error);
        }
        // Local fallback
        ...
        errorLogger_1.ErrorLogger.info('[Storage] Contact created locally', { id, phone: contact.phone });
        return { id, ...contact };
    }
    async updateContact(id, updates) {
        try {
            if (this.isCloudAvailable()) {
                ...
                if (error)
                    throw error;
                errorLogger_1.ErrorLogger.info('[Storage] Contact updated in cloud', { id });
                return;
            }
        }
        catch (error) {
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud update failed, using local fallback', error);
        }
        // Local fallback
        db.contacts.update(id, updates);
        errorLogger_1.ErrorLogger.info('[Storage] Contact updated locally', { id });
    }
    async deleteContact(id) {
        try {
            if (this.isCloudAvailable()) {
                ...
                if (error)
                    throw error;
                errorLogger_1.ErrorLogger.info('[Storage] Contact deleted from cloud', { id });
                return;
            }
        }
        catch (error) {
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud delete failed, using local fallback', error);
        }
        // Local fallback
        db.contacts.delete(id);
        errorLogger_1.ErrorLogger.info('[Storage] Contact deleted locally', { id });
    }
    ...
    async getGroups() {
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud fetch failed for groups', error);
        }
    ...
    async createGroup(group) {
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud create failed for group', error);
        }
    ...
    async getCampaigns() {
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud fetch failed for campaigns', error);
        }
    ...
    async getCampaignWithMessages(id) {
        ...
        try {
            ...
            if (messages.length === 0) {
                ...
                let contacts = db.campaigns.getContacts(id);
            ...
            const mediaAttachments = mediaFiles.map((f) => ({
                path: f.file_path,
                type: f.file_type,
            ...
            errorLogger_1.ErrorLogger.error('[Storage] Failed to get campaign with messages', error);
            return null;
        }
    }
    async createCampaign(campaign) {
        ...
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud create failed for campaign', error);
        }
        ...
        if (campaign.template_image_data) {
            try {
                ...
                errorLogger_1.ErrorLogger.info('[Storage] Template image saved locally:', filePath);
            }
            catch (err) {
                errorLogger_1.ErrorLogger.error('[Storage] Failed to save template image:', err);
            }
        }
    ...
    async updateCampaign(id, updates) {
        ...
        if (updates.rotation_enabled) {
            ...
            if (!updates.sending_strategy)
                updates.sending_strategy = 'single';
        }
        try {
            if (this.isCloudAvailable()) {
                const { error } = await this.supabase
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud update failed for campaign', error);
        }
        if (updates.template_image_data) {
            try {
                ...
                errorLogger_1.ErrorLogger.info('[Storage] Template image updated locally:', filePath);
            }
            catch (err) {
                errorLogger_1.ErrorLogger.error('[Storage] Failed to update template image:', err);
            }
        }
    ...
    async updateCampaignProgress(id, sentCount, failedCount, status) {
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud updateProgress failed', error);
        }
    ...
    async deleteCampaign(id) {
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud delete failed for campaign', error);
        }
    ...
    async getCampaignMedia(campaignId) {
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud fetch failed for campaign media', error);
        }
    ...
    async createCampaignMedia(media) {
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud create failed for campaign media', error);
        }
        ...
        if (media.fileData || media.file_data) {
            try {
                ...
                errorLogger_1.ErrorLogger.info('[Storage] Campaign media saved locally:', filePath);
            }
            catch (err) {
                errorLogger_1.ErrorLogger.error('[Storage] Failed to save campaign media:', err);
            }
        }
    ...
    async deleteCampaignMedia(id) {
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud delete failed for campaign media', error);
        }
    ...
    async updateCampaignMessageStatus(messageId, status, errorMsg) {
        try {
            ...
            errorLogger_1.ErrorLogger.warn('[Storage] Cloud updateCampaignMessageStatus failed', error);
        }
