...
function personalizeMessage(template, variables, contactName, contactPhone) {
    if (!template)
        return '';
    ...
    return template.replace(/\{\{\s*([^}]+)\s*\}\}/g, (_match, key) => {
        ...
        if (varName === 'name')
            return contactName || '';
        if (varName === 'phone' || varName === 'mobile')
            return contactPhone || '';
...
async function executeCampaignAsync(campaignTask, whatsAppClient) {
    campaignShouldStop = false;
    let sentCount = 0;
    let failedCount = 0;
    const totalMessages = campaignTask.messages?.length || 0;
    const failedMessages = []; // Track failed messages for download
    ...
    for (let i = 0; i < totalMessages; i++) {
        ...
        try {
            ...
            const content = personalizeMessage(message.templateText || '', message.variables || {}, message.recipientName || '', message.recipientNumber || '');
                ...
                for (const media of message.mediaAttachments) {
                    try {
                        if (media.file_path) {
                            ...
                            const mediaCaption = personalizeMessage(rawCaption, message.variables || {}, message.recipientName, message.recipientNumber);
...
export function registerIpcHandlers(mainWindow) {
    ...
    try {
        ...
        ipcMain.handle('campaign:start', async (_event, campaignTask) => {
