...
export class StorageService {
  ...
  async updateContact(id: number, updates: any): Promise<void> {
    try {
      if (this.isCloudAvailable()) {
        const updateData: any = {};
        if (updates.phone) updateData.phone = updates.phone;
        if (updates.name) updateData.name = updates.name;
        if (updates.variables) updateData.vars_json = JSON.stringify(updates.variables);
  ...
  async createGroup(group: { name: string }): Promise<any> {
    try {
      if (this.isCloudAvailable()) {
        const { data, error } = await this.supabase!
          .from('groups')
  ...
  async updateCampaignProgress(id: number, sentCount: number, failedCount: number, status?: string): Promise<void> {
    try {
      if (this.isCloudAvailable()) {
        const updates: any = { sent_count: sentCount, failed_count: failedCount };
   ...
   */
  getSupabaseCredentials(): { url: string; key: string } | null {
   ...
   */
  async getCampaignMedia(campaignId: number): Promise<any[]> {
    try {
  ...
  async createCampaignMedia(media: any): Promise<void> {
    try {
      if (this.isCloudAvailable() && this.mode !== 'local') {
        const { error } = await this.supabase!
          .from('campaign_media')
    ...
    if (media.campaignId && !media.campaign_id) media.campaign_id = media.campaignId;
  ...
  async updateCampaignMessageStatus(messageId: string, status: 'sent' | 'failed', errorMsg?: string): Promise<void> {
    try {
      if (this.isCloudAvailable()) {
