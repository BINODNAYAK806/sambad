...
export function getSentinelConfig(): { url: string; key: string } {
...
export function saveSupabaseConfig(supabaseUrl: string, supabaseKey: string): boolean {
  try {
    const configPath = getConfigPath();
    const userDataPath = app.getPath('userData');
...
export function clearSupabaseConfig(): boolean {
  try {
    const configPath = getConfigPath();
...
export function validateSupabaseCredentials(supabaseUrl: string, supabaseKey: string): { valid: boolean; error?: string } {
...
export function getSupabaseStatus(): { configured: boolean; supabaseUrl?: string; savedAt?: string } {
