...
export class ServerHealthMonitor {
    ...
    constructor(private whatsAppClient: WhatsAppClientSingleton) { }
     ...
     */
    async waitForServer(serverId: number, timeout: number = 30000): Promise<void> {
     ...
     */
    async waitForAnyServer(timeout: number = 30000): Promise<number> {
        const startTime = Date.now();
