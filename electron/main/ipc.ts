...
export function registerIpcHandlers(mainWindow: Electron.BrowserWindow | null) {
  ...
  try {
    ...
    const withPermission = (module: ModuleName, action: PermissionAction, handler: (...args: any[]) => Promise<any>, schema?: z.ZodSchema) => {
      return async (event: any, ...args: any[]) => {
        ...
        if (!isLocal) {
          ...
          if (!isValid) {
            ...
            if (mainWindow) mainWindow.webContents.send('auth:session-invalidated', { message: 'Session expired or active elsewhere' });
    ...
    ipcMain.handle('contacts:create', withPermission('contacts', 'create', async (_event, contact: any) => {
      try {
    ...
    ipcMain.handle('contacts:update', withPermission('contacts', 'update', async (_event, id: number, contact: any) => {
      try {
    ...
    ipcMain.handle('contacts:delete', withPermission('contacts', 'delete', async (_event, id: number) => {
      try {
    ...
    ipcMain.handle('contacts:bulkCreate', withPermission('contacts', 'create', async (_event, contactsList: any[]) => {
      try {
    ...
    ipcMain.handle('campaigns:create', withPermission('campaigns', 'create', async (_event, campaign: any) => {
      try {
    ...
    ipcMain.handle('campaigns:update', withPermission('campaigns', 'update', async (_event, id: number, campaign: any) => {
      try {
    ...
    ipcMain.handle('campaigns:delete', withPermission('campaigns', 'delete', async (_event, id: number) => {
      try {
    ...
    ipcMain.handle('campaignRuns:getByid', withPermission('campaigns', 'read', async (_event, runId: number) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('campaignRuns:getFailedMessages', withPermission('campaigns', 'read', async (_event, runId: number) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('groups:findOrCreate', withPermission('contacts', 'create', async (_event, name: string) => {
      try {
    ...
    ipcMain.handle('groups:create', withPermission('contacts', 'create', async (_event, group: any) => {
      try {
    ...
    ipcMain.handle('groups:update', withPermission('contacts', 'update', async (_event, id: number, groupUpdates: any) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('groups:delete', withPermission('contacts', 'delete', async (_event, id: number) => {
      try {
        const db = await import('./db/index.js');
        db.groups.delete(id);
    ...
    ipcMain.handle('groups:addContact', withPermission('contacts', 'update', async (_event, groupId: number, contactId: number) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('groups:bulkAddContactsToMultipleGroups', withPermission('contacts', 'update', async (_event, groupIds: number[], contactIds: number[]) => {
      try {
    ...
    ipcMain.handle('groups:removeContact', withPermission('contacts', 'update', async (_event, groupId: number, contactId: number) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('groups:getContacts', withPermission('contacts', 'read', async (_event, groupId: number, skipMasking = false) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('groups:bulkAddContacts', withPermission('contacts', 'update', async (_event, groupId: number, contactIds: number[]) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('campaigns:addMedia', withPermission('campaigns', 'update', async (_event, campaignId: number, media: any) => {
      try {
    ...
    ipcMain.handle('campaigns:addContact', withPermission('campaigns', 'update', async (_event, campaignId: number, contactId: number) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('campaigns:addContacts', withPermission('campaigns', 'update', async (_event, campaignId: number, contactIds: number[]) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('campaigns:removeContact', withPermission('campaigns', 'update', async (_event, campaignId: number, contactId: number) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('campaigns:clearContacts', withPermission('campaigns', 'update', async (_event, campaignId: number) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('campaigns:getContacts', withPermission('campaigns', 'read', async (_event, campaignId: number, skipMasking = false) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('campaigns:getMedia', withPermission('campaigns', 'read', async (_event, campaignId: number) => {
      try {
    ...
    ipcMain.handle('campaigns:deleteMedia', withPermission('campaigns', 'update', async (_event, mediaId: string) => {
      try {
    ...
    ipcMain.handle('campaigns:exportFailureReport', withPermission('campaigns', 'read', async (_event, campaignId: number, campaignName: string) => {
      try {
    ...
    ipcMain.handle('campaigns:getFailedMessages', withPermission('campaigns', 'read', async (_event, campaignId: number) => {
      try {
        const db = await import('./db/index.js');
        let messages: any[] = [];
        ...
        messages = messages.map(msg => {
          let rawError = msg.error_message || 'Unknown';
          if (rawError.includes('\n')) rawError = rawError.split('\n')[0];
          if (rawError.startsWith('Evaluation failed: ')) rawError = rawError.replace('Evaluation failed: ', '');
...
`;
    ...
    ipcMain.handle('logs:list', withPermission('logs', 'read', async (_event, limit?: number) => {
      try {
        const db = await import('./db/index.js');
    ...
    ipcMain.handle('logs:add', withPermission('logs', 'create', async (_event, log: any) => {
      try {
        const db = await import('./db/index.js');
        db.logs.create(log);
        return { success: true };
    ...
    ipcMain.handle('reports:generate', withPermission('reports', 'read', async (_event, _params: any) => {
      try {
    ...
    ipcMain.handle('supabase:saveConfig', async (_event, supabaseUrl: string, supabaseKey: string) => {
      try {
    ...
    // --- Auth Handlers ---
    ipcMain.handle('auth:register', async (_event, data: RegistrationData) => {
      try {
    ...
    ipcMain.handle('auth:forgotPassword', async (_event, email: string) => {
      try {
    ...
    ipcMain.handle('subscription:getStatus', async () => {
      const session = authService.getCurrentSession();
      if (!session) return { isActive: false, plan: 'EXPIRED', endDate: null };
      try {
    ...
    ipcMain.handle('chromium:setPath', async (_event, chromiumPath: string) => {
      try {
